...
<Columns cols="2">
  <Card title="Supabase Auth" href="/technical/auth/supabase-auth" icon="lock" horizontal={false}>
    Session management, user profiles, and automatic logout configuration.
  </Card>

  <Card title="Microsoft OAuth" href="/technical/auth/microsoft-oauth" icon="mail" horizontal={false}>
    OAuth2 flow for Outlook email integration.
  </Card>

  <Card title="RBAC" href="/technical/auth/rbac" icon="users" horizontal={false}>
    Role definitions, access matrix, and navigation filtering.
  </Card>

  <Card title="RLS policies" href="/technical/database/rls-policies" icon="shield" horizontal={false}>
    Database-level access control policies for all tables.
  </Card>
</Columns>
