...
<DataTable
  columns={[
    ...
    { key: "status", label: t("status"), render: (row) => (
      <StatusBadge status={row.status_nl} />
    )},
  ]}
  data={trailers}
...
Every component has a corresponding `.test.tsx` file that verifies rendering, user interactions, and edge cases using Testing Library. See [Testing](/technical/development/testing) for details on the test infrastructure.
