...
<Steps>
  <Step title="Open Template Management" icon="file-text" titleType="p">
    Navigate to **Administration > Templates** in the main menu.
  </Step>

  <Step title="Select the company" icon="building" titleType="p">
    Choose the company (Atrac or Urbain) for which you want to upload a template.
  </Step>

  <Step title="Choose the document type and language" icon="languages" titleType="p">
    Select whether the template is for an **offer** or a **contract**, and choose the language (**NL** or **FR**).
  </Step>

  <Step title="Upload the template file" icon="upload" titleType="p">
    Click **Upload** and select your template file. Templates can be in Word or HTML format.

    <Callout kind="alert">
      The maximum file size for uploads is controlled by the `MAX_UPLOAD_SIZE_MB` parameter (default: 25 MB). See [Parameters](/user-guide/administration/parameters) for details.
    </Callout>
  </Step>

  <Step title="Preview the template" icon="eye" titleType="p">
  ...
  <Step title="Set as default (optional)" icon="star" titleType="p">
