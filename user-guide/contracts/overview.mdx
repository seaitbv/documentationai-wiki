...
<Callout kind="info">
  For the full lifecycle including automatic transitions and side effects, see [Contract lifecycle](/user-guide/contracts/lifecycle).
</Callout>
...
| **Yellow background** | Trailer has not been inspected after return, or damage has been found | Complete the [damage control](/user-guide/contracts/damage-control) process |

<Callout kind="tip">
  Yellow-highlighted contracts also appear with the same color on the [planning timeline](/user-guide/planning/overview), so you can spot them from multiple screens.
</Callout>
...
Click the **Add contract** button in the top-right corner to open the contract creation form. You can also create a contract directly from an accepted offer. See [Creating a contract](/user-guide/contracts/creating-contract) for detailed steps.
...
<Columns cols="2">
  <Card title="Creating a contract" href="/user-guide/contracts/creating-contract" icon="file-plus" horizontal="false">
    Step-by-step guide to creating a new rental contract.
  </Card>

  <Card title="Contract lifecycle" href="/user-guide/contracts/lifecycle" icon="git-branch" horizontal="false">
    Understand the full status flow from creation to completion.
  </Card>
</Columns>
