...
- Both invoices appear in the [invoice overview](/user-guide/invoicing/overview) with status "Created"

See [Deposits and advances](/user-guide/contracts/deposits-advances) for how these amounts are calculated.
...
When you send a contract via email, the status changes from "Created" to "Sent". See [Sending via Outlook](/user-guide/contracts/sending-outlook) for the detailed process.
...
<Callout kind="tip">
  After marking a contract as "To check", complete the [damage control](/user-guide/contracts/damage-control) checklist before marking the contract as completed.
</Callout>
...
These visual indicators help you identify contracts that need follow-up. See [Damage control](/user-guide/contracts/damage-control) for resolving yellow-highlighted contracts.

## Related pages

<Columns cols="2">
  <Card title="Damage control" href="/user-guide/contracts/damage-control" icon="alert-triangle" horizontal="false">
    Handle post-return inspection and damage tracking.
  </Card>

  <Card title="Deposits and advances" href="/user-guide/contracts/deposits-advances" icon="banknote" horizontal="false">
    Understand the automatic invoice creation when a contract enters "In rental".
  </Card>
</Columns>
